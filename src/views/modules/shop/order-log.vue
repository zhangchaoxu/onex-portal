  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-shop__order-log">
      <el-form :inline="true" :model="searchDataForm" size="small" @submit.native.prevent>
        ...
        <el-form-item>
          <el-input v-model="searchDataForm.orderNo" placeholder="订单号" clearable/>
        </el-form-item>
        <el-form-item>
          <el-button @click="queryDataList()">{{ $t('query') }}</el-button>
        </el-form-item>
        <el-form-item v-if="$hasPermission('shop:orderLog:export')">
          <el-button type="info" @click="exportHandle()">{{ $t('export') }}</el-button>
        </el-form-item>
        <el-form-item v-if="$hasPermission('shop:orderLog:delete')">
      ...
      <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" @sort-change="dataListSortChangeHandle" style="width: 100%;">
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('shop:orderLog:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
  ...
  data () {
    return {
      mixinListModuleOptions: {
        getDataListURL: '/shop/orderLog/page',
        getDataListIsPage: true,
        exportURL: '/shop/orderLog/export',
        deleteURL: '/shop/orderLog/delete',
        deleteBatchURL: '/shop/orderLog/deleteBatch',
        deleteIsBatch: true
      },
      searchDataForm: {
        orderNo: '',
